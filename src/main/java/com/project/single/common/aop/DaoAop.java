package com.project.single.common.aop;

import com.project.single.doentity.common.BaseDo;
...
public class DaoAop {
     ...
     */
    @Pointcut("execution(public * com.project.single.mapper..*.insert*(..))")
    public void insertBaseData(){}

    /**
     * 拦截跟新操作  维护基本信息 例如 创建人、创建时间
     */
    @Pointcut("execution(public * com.project.single.mapper..*.update*(..))")
    ...
    public void doBeforeUpate(JoinPoint joinPoint) throws Throwable {
        if (joinPoint.getArgs() !=  null && joinPoint.getArgs().length > 0) {
            //获取当前人员信息
            UserDo userEntity=getUserData();
            for ( int i = 0; i < joinPoint.getArgs().length; i++) {
                Object obj= joinPoint.getArgs()[i];
                if(obj.getClass().getSuperclass().equals(BaseDo.class)){
                    //注入参数
                    Class[] types = {UserDo.class};
                    //根据方法签名在cls查找方法的信息
